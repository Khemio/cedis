...
grow(Dict d)
{
    ...
    for(i = 0; i < d->size; i++) {
        for(e = d->table[i]; e != 0; e = e->next) {
            ...
            DictInsert(d2, e->key, e->value, e->expiry, e->expire_at);
        }
    }
...
void
DictInsert(Dict d, const char *key, const char *value, int expiry, long expire_at)
{
    struct elt *e;
    unsigned long h;
...
// const char *
int
DictSearch(Dict d, response *res, const char *key)
{
    struct elt *e;

    for(e = d->table[hash_function(key) % d->size]; e != 0; e = e->next) {
        if(!strcmp(e->key, key)) {
            res->value = strdup(e->value);
            res->expire_at = e->expire_at;
            res->expiry = e->expiry;
            /* got it */
            // return e->value;
            return 1;
